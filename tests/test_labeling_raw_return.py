...
from quantinaut.labels.raw_return import raw_return

@pytest.fixture(scope="module")
def stock_data():
    project_path = os.path.dirname(__file__)
    path = os.path.join(project_path, "test_data", "stock_prices.csv")
    data = pd.read_csv(path, index_col="Date", parse_dates=True)
    idx5 = data[:5].index
    col5 = data.iloc[:, 0:5].columns
    return data, idx5, col5

def test_labeling_raw_returns_dataframe(stock_data):
    data, idx5, col5 = stock_data
    prices = data.iloc[0:5, 0:5]
...
def test_labeling_raw_returns_series():
...
def test_labeling_raw_returns_resample(stock_data):
    data, _, col5 = stock_data
    price1 = data.iloc[0:25, 0:5]
    price2 = data.iloc[:, 0:3]
    week_index = price1.resample("W").last().index
    year_index = data.resample("YE").last().index
