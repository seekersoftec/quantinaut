...
from nautilus_trader.model.currencies import BTC, ETH, SOL, XRP, ADA
from nautilus_trader.model.currencies import AUD, GBP, USD
...
class TestInstrumentProvider(InstrumentProvider):
    ...
    def solusdt_perp_binance() -> CryptoPerpetual:
        ...
        return CryptoPerpetual(
            ...
            price_precision=2,
            size_precision=3,            
            price_increment=Price.from_str("0.01"),        # per spec
            size_increment=Quantity.from_str("0.001"),     # per spec
            max_quantity=None,                             # deprecated—use max_notional
            min_quantity=Quantity.from_str("0.001"),
            max_notional=Money(2_500_000, USDT),           # per platform max
            min_notional=Money(10.00, USDT),               # per platform min
            max_price=None,                                # not strictly needed
            min_price=Price(0.01, precision=2),
            margin_init=Decimal("0.02"),                   # 2% initial margin per spec
            margin_maint=Decimal("0.006"),                 # 0.6% maintenance
            maker_fee=Decimal("0.0002"),
            taker_fee=Decimal("0.0004"),
            ts_event=1646199312128000000,
            ts_init=1646199342953849862,
