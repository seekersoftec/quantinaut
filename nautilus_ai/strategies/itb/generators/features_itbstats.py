# generators/features_itbstats.py

import numpy as np
import pandas as pd
from scipy import stats
from nautilus_ai.strategies.itb.generators.generator import Generator, register_generator
from nautilus_ai.strategies.itb.generators.utils import _aggregate_last_rows, _convert_to_relative, area_fn, fmax_fn, lsbm_fn, slope_fn


@register_generator("itbstats")
class ITBStats(Generator):
    def generate(self, df, last_rows=0, **kwargs):
        return _itbstats(df, self.config, last_rows)




def _itbstats(df, config: dict, last_rows: int = 0):
    """
