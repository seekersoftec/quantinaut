# generators/features_depth.py

import numpy as np
import pandas as pd
from nautilus_ai.strategies.itb.generators.utils import add_past_aggregations
from nautilus_ai.strategies.itb.generators.generator import Generator, register_generator


@register_generator("depth")
class Depth(Generator):
    """
    Market Depth/Order Book features generator
    """
    def generate(self, df, last_rows=0, **kwargs):
        return _depth(df, kwargs.get("use_differences", False))



def _depth(df, use_differences=False):
    """
