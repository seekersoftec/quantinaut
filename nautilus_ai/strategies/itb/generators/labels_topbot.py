...
@register_generator("topbot")
class TopBot(LabelGenerator):
    """
    Generates multiple top/bottom extremum labels using preconfigured levels and tolerances.
    """
    def generate(self, df: pd.DataFrame, last_rows: int = 0, **kwargs) -> List[str]:
        ...
        Parameters:
            ...
            last_rows (int): Not used in this generator, but included for compatibility.
            **kwargs:
                column_name (str): Name of the price column to use.
                top_level_fracs (list): List of fractional levels to identify top extrema.
                bot_level_fracs (list): List of fractional levels to identify bottom extrema.

        Returns:
            List[str]: List of generated label column names.
        """
        init_column_number = len(df.columns)

        column_name = kwargs.get("column_name", "")
        top_level_fracs = kwargs.get("top_level_fracs", [])
        bot_level_fracs = kwargs.get("bot_level_fracs", [])

        if not column_name or not isinstance(column_name, str):
            raise ValueError("column_name must be provided as a string.")
        if not isinstance(top_level_fracs, list) or not isinstance(bot_level_fracs, list):
            raise ValueError("top_level_fracs and bot_level_fracs must be lists.")
...
@register_generator("topbot2")
class TopBot2(Generator):
    """
    Generates top/bottom labels using a specific level and multiple tolerance values.
    """
    def generate(self, df: pd.DataFrame, last_rows: int = 0) -> List[str]:
        """Find and label top points."""
        init_column_number = len(df.columns)

        column_name = self.config.get('columns')
        if not column_name:
        ...
        tolerances = self.config.get('tolerances')  # For example, 0.0025 for 0.25% tolerance
        if not isinstance(tolerances, list):
            tolerances = [tolerances]

        level = self.config.get('level')  # For example, 0.01 for 1% (positive or negative)
        if function == 'top':
            level = abs(level)
        elif function == 'bot':
            level = -abs(level)

        names = self.config.get('names')  # For example, ['top1_025', 'top1_01'] for two tolerances
