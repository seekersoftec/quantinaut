...
class INautilusAIModel(Actor, ABC):
    ...
    def __init__(self, config: INautilusAIModelConfig) -> None:
        ...
        """
            TODO: the config here is not a file but a class 
        """

        self.assert_config(self.config)
        self.nautilus_ai_info: dict[str, Any] = config["nautilus_ai"]
        self.data_split_parameters: dict[str, Any] = config.get("nautilus_ai", {}).get(
            "data_split_parameters", {}
        )
        self.model_training_parameters: dict[str, Any] = config.get(
            "nautilus_ai", {}
        ).get("model_training_parameters", {})

        self.identifier: str = self.nautilus_ai_info.get("identifier", "no_id_provided")
        self.retrain = False
        self.first = True
        self.set_full_path()

        self.save_backtest_models: bool = self.nautilus_ai_info.get(
            "save_backtest_models", True
        )
        if self.save_backtest_models:
            logger.info("Backtesting module configured to save all models.")

        self.dd = NautilusAIDataDrawer(Path(self.full_path), self.config)
        ...
        self.scanning = False
        self.ft_params = self.nautilus_ai_info["feature_parameters"]
        self.corr_pairlist: list[str] = self.ft_params.get("include_corr_pairlist", [])
        self.keras: bool = self.nautilus_ai_info.get("keras", False)
        ...
        self.CONV_WIDTH = self.nautilus_ai_info.get("conv_width", 1)
        ...
        self.continual_learning = self.nautilus_ai_info.get("continual_learning", False)
        ...
        self.activate_tensorboard: bool = self.nautilus_ai_info.get(
    ...
    # def assert_config(self, config: INautilusAIModelConfig) -> None:
    #     """
    #     TODO: Fix this, the config is coming from a class instead of a file
    #     """
    #     if not config.get("freqai", {}):
    #         raise OperationalException("No parameters found in configuration.")
    ...
    def start(
        ...
        if self.live:
            ...
            self.dk = NautilusAIDataKitchen(self.config, self.live, metadata["pair"])
    ...
    def shutdown(self):
        ...
        if self.nautilus_ai_info.get("wait_for_training_iteration_on_reload", True):
    ...
    def _start_scanning(self, strategy: IStrategy) -> None:
        ...
        while not self._stop_event.is_set():
            ...
            dk = NautilusAIDataKitchen(self.config, self.live, pair)
            (
                retrain,
            ...
            if retrain:
                ...
                self.dd.save_historic_predictions_to_disk()
                if self.nautilus_ai_info.get("write_metrics_to_disk", False):
    ...
    def start_backtesting(
        ...
        :param dk: NautilusAIDataKitchen = Data management/analysis tool associated to present pair only
        :param strategy: Strategy to train on
        :return:
            NautilusAIDataKitchen = Data management/analysis tool associated to present pair only
        """
    ...
    def start_live(
        ...
        dk: NautilusAIDataKitchen = Data management/analysis tool associated to present pair only
        :returns:
        dk: NautilusAIDataKitchen = Data management/analysis tool associated to present pair only
        """
    ...
    def build_strategy_return_arrays(
        ...
        if self.nautilus_ai_info.get("fit_live_predictions_candles", 0) and self.live:
    ...
    def check_if_feature_list_matches_strategy(self, dk: NautilusAIDataKitchen) -> None:
        """
        Ensure user is passing the proper feature set if they are reusing an `identifier` pointing
        to a folder holding existing models.
        :param dataframe: DataFrame = strategy provided dataframe
        :param dk: NautilusAIDataKitchen = non-persistent data container/analyzer for
    ...
    def define_data_pipeline(self, threads=-1) -> Pipeline:
        ft_params = self.nautilus_ai_info["feature_parameters"]
        pipe_steps = [
        ...
        sigma = self.nautilus_ai_info["feature_parameters"].get(
    ...
    def model_exists(self, dk: NautilusAIDataKitchen) -> bool:
        """
    ...
    def extract_data_and_train_model(
        ...
        :param dk: NautilusAIDataKitchen = non-persistent data container for current coin/loop
    ...
    def set_initial_historic_predictions(
        ...
        :param pred_df: DataFrame = the dataframe containing the predictions coming
            out of a model
        :param dk: NautilusAIDataKitchen = object containing methods for data analysis
        ...
        if self.nautilus_ai_info["feature_parameters"].get("DI_threshold", 0) > 0:
    ...
    def fit_live_predictions(self, dk: NautilusAIDataKitchen, pair: str) -> None:
        """
        ...
        num_candles = self.nautilus_ai_info.get("fit_live_predictions_candles", 100)
    ...
    def _set_train_queue(self):
        ...
        if not self.dd.pair_dict:
            logger.info(
                "Set fresh train queue from whitelist. Queue: {current_pairlist}"
            )
    ...
    def cache_corr_pairlist_dfs(
        self, dataframe: DataFrame, dk: NautilusAIDataKitchen
    ) -> DataFrame:
        """
    ...
    def backtesting_fit_live_predictions(self, dk: NautilusAIDataKitchen):
        """
        ...
        """
        fit_live_predictions_candles = self.nautilus_ai_info.get(
    ...
    def start_backtesting_from_historic_predictions(
        self, dataframe: DataFrame, metadata: dict, dk: NautilusAIDataKitchen
    ) -> NautilusAIDataKitchen:
        """
        :param dataframe: DataFrame = strategy passed dataframe
        :param metadata: Dict = pair metadata
    ...
    @abstractmethod
    def fit(
        self, data_dictionary: dict[str, Any], dk: NautilusAIDataKitchen, **kwargs
    ) -> Any:
        """
